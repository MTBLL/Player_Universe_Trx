...
def savant_fixture_path():
    ...
    fixtures_dir = Path(__file__).parent / "fixtures"
    return fixtures_dir / "savant_batters_2026_05_14_0516.json"
...
def savant_batter_data() -> List[Dict]:
    ...
    fixtures_path = Path(__file__).parent / "fixtures"
    with open(fixtures_path / "savant_batters_2026_05_14_0516.json") as f:
        data = json.load(f)
    return data[:10]
...
def savant_pitcher_data() -> List[Dict]:
    ...
    fixtures_path = Path(__file__).parent / "fixtures"
    with open(fixtures_path / "savant_pitchers_2026_05_14_0516.json") as f:
        data = json.load(f)
    return data[:10]
