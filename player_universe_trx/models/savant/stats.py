...
class SavantSwingTakeModel(BaseModel):
    """Swing/take run-value by plate region, shared shape for batters and pitchers.

    Sourced from `savant_swing_take_{batter,pitcher}_*.json`. The `runs_*`
    fields decompose total run value into Statcast's four plate regions:
      - heart:  middle of the strike zone
      - shadow: the edges (in-and-around the zone boundary)
      - chase:  just off the plate
      - waste:  well outside the zone
    For a batter these are the run value of their swing/take decisions in each
    region; for a pitcher, the run value generated by locating there.
    `runs_all` is the total across regions.
    """

    model_config = SAVANT_STATS_MODEL_CONFIG

    year: Optional[int] = Field(default=None, description="Season")
    team_id: Optional[int] = Field(default=None, description="Statcast team id")
    PA: Optional[int] = Field(default=None, description="Plate appearances in sample")
    pitches: Optional[int] = Field(default=None, description="Pitches seen/thrown in sample")
    runs_all: Optional[float] = Field(default=None, description="Total swing/take run value")
    runs_heart: Optional[float] = Field(default=None, description="Run value in the heart of the zone")
    runs_shadow: Optional[float] = Field(default=None, description="Run value in the shadow (zone edges)")
    runs_chase: Optional[float] = Field(default=None, description="Run value in the chase region (just off the plate)")
    runs_waste: Optional[float] = Field(default=None, description="Run value in the waste region (well outside)")
